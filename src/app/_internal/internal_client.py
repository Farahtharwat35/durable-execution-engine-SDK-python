...
class InternalEndureClient:
    ...
    def send_log(self, execution_id: str, log: Log, action_name: str):
        ...
        if not self._base_url:
            raise ValueError("DURABLE_ENGINE_BASE_URL is not set in environment variables.")
        
        if not execution_id or not log or not action_name:
            raise ValueError("execution_id, log, and action_name must be provided.")
        ...
        response = Response(status=response.status_code, payload=response.json())
