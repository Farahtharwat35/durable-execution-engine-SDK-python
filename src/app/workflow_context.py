...
class WorkflowContext:
    ...
    def execute_action(self, action: callable, input_data , max_retries: int , retry_mechanism: RetryMechanism) -> any:
        """
        ...
        """
        try:
            log = Log(status=LogStatus.STARTED, input=input_data, retry_mechanism=retry_mechanism, max_retries=max_retries)
            engine_response = InternalEndureClient.send_log(self.execution_id , log , action.__name__)
            ...
            match status_code:
                case status.HTTP_201_CREATED | status.HTTP_200_OK:
                    ...
                    log = Log(status=LogStatus.COMPLETED, output=result)
                    InternalEndureClient.send_log(self.execution_id, log, action.__name__)
                    return result 
            ...
            raise RuntimeError(f"Action execution failed: {str(e)} , status code: {e.status_code}") 
                
        except Exception as e:
            log = Log(status=LogStatus.FAILED, output={"error": str(e)})
            engine_response = InternalEndureClient.send_log(self.execution_id, log, action.__name__)
            status_code = engine_response.status_code
